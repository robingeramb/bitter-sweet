...
/* ---------------- STATES ---------------- */
const stage = ref<"intro" | "waiting" | "info" | "final" | "completed" | "completed_check">(
  "intro"
);
...
const currentDisplayText = computed(() => {
  if (stage.value === "final") return "So, now we take a deep dive into your body to see the other consequences";
...
.fade-enter-active {
  transition: opacity 1s ease-out, transform 1s cubic-bezier(0.19, 1, 0.22, 1), filter 1s ease-out;
}
