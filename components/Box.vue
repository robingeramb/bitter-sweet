...
/* --- Imports --- */
import { useThree, camera, scene, playerMaterial, debugMeshes, raycaster, mouse, clock, _composer, productSelection, taskDone, selectMode, hoveredProduct, hoveredMouseX, hoveredMouseY, selectedProduct, productView, world, usePlayerBody } from "@/composables/useThree"; // NEU: Zentrales Material importieren
...
import { defineProps, defineExpose, ref, computed, watch, onMounted, onBeforeUnmount } from "vue";
...
function checkIntersects() {
  ...
  if (productView.value) {
    if (selectedProduct.value) intersects = raycaster.intersectObjects([selectedProduct.value]);
...
function setupFloor(): void {
  const ceramicMaterial = createTexture( // @ts-ignore
    "ceramic_tiles",
    16,
    true,
    true,
  ...
  const floorGeometry = new THREE.BoxGeometry(floorLength + 6, 0.1, floorLength + 6);
...
function setupFloorPhysics(): void {
  const floorShape = new CANNON.Box(new CANNON.Vec3((floorLength + 6) / 2, 0.1 / 2, (floorLength + 6) / 2));
  ...
  console.log(`DEBUG: Floor Body created at Y: ${floorBody.position.y}, Half Extents Y: ${floorShape.halfExtents.y}`);

  // DEBUG: Erstelle ein sichtbares Mesh für die Kollisionsbox des Bodens
  const debugFloorMaterial = new THREE.MeshBasicMaterial({ color: 0x00ff00, wireframe: true, transparent: true, opacity: 0.5, visible: false });
  const floorDebugMesh = new THREE.Mesh(
    new THREE.BoxGeometry(floorShape.halfExtents.x * 2, floorShape.halfExtents.y * 2, floorShape.halfExtents.z * 2),
    debugFloorMaterial
  );
...
function setupRoof(): void {
  ...
  const roofGeometry = new THREE.BoxGeometry(floorLength + 6, 0.1, floorLength + 6);
...
function setupWalls(): void {
  ...
  const wallGeometry = new THREE.BoxGeometry(0.1, floorLength + 4, floorLength + 4);
...
async function setupShoppingCart(shoplight: any): Promise<void> {
  const metal = createTexture( // @ts-ignore
    "shoppingCart",
    1,
    true,
    false,
...
function renderLoop(): void {
  ...
  if (shoppingCart) {
    ...
    const isMoving =
      fpControls &&
      (fpControls.moveState.forward || fpControls.moveState.backward || fpControls.moveState.left || fpControls.moveState.right);
...
onMounted(() => {
  if (canvas.value) {
    ...
    const cylinderShape = new CANNON.Cylinder(playerRadius, playerRadius, playerHeight - 2 * playerRadius, 8);
    ...
    playerBody.addShape(cylinderShape, new CANNON.Vec3(0, (playerHeight - playerRadius) / 2, 0)); // Zylinder über der Kugel
    ...
    console.log("DEBUG: Player Body erstellt.", { id: playerBody.id, material: playerBody.material, group: playerBody.collisionFilterGroup, mask: playerBody.collisionFilterMask });

    const playerDebugMaterial = new THREE.MeshBasicMaterial({ color: 0xff0000, wireframe: true, transparent: true, opacity: 0.5, visible: false });
    // Erstelle ein Debug-Mesh, das die Kapselform widerspiegelt
    const playerDebugMesh = new THREE.Group();
    const sphereMesh = new THREE.Mesh(new THREE.SphereGeometry(playerRadius, 8, 8), playerDebugMaterial);
    const cylinderMesh = new THREE.Mesh(new THREE.CylinderGeometry(playerRadius, playerRadius, playerHeight - 2 * playerRadius, 8), playerDebugMaterial);
