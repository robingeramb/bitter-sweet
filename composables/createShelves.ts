...
import { shelfMaterial as shelfPhysicMaterial, debugMeshes } from "@/composables/useThree"; // KORREKTUR: Import umbenennen und zentrale debugMeshes importieren
...
export async function createShelves(
  ...
  for (let index = 0; index < myStore.shelves.length; index++) {
    ...
    const shelf = await createShelve( // KORREKTUR: shelf muss hier deklariert werden
      shelfHeight,
      shelfWidth,
      shelfLength,
    ...
    const shelfShape = new CANNON.Box(new CANNON.Vec3(shelfLength / 2, shelfHeight / 2, shelfWidth / 2));
    ...
    console.log(`DEBUG: Regal-Körper ${index} erstellt.`, { id: shelfBody.id, material: shelfBody.material, group: shelfBody.collisionFilterGroup, mask: shelfBody.collisionFilterMask });

    // DEBUG: Erstelle ein sichtbares Mesh für die Kollisionsbox des Regals
    const debugMaterial = new THREE.MeshBasicMaterial({ color: 0xff0000, wireframe: true, transparent: true, opacity: 0.5, visible: false });
