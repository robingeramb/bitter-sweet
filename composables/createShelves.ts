...
export async function createShelves(
  ...
  for (let index = 0; index < myStore.shelves.length; index++) {
    if (index % 2 === 0) {
      // Gerade Indizes (0, 2, ...) platzieren ein Regal auf der rechten Seite.
      currPos = pos.x2;
      posCounter++;
    } else {
      // Ungerade Indizes (1, 3, ...) platzieren ein Regal auf der linken Seite.
      currPos = pos.x1;
    }
    ...
    (shelfBody as any).name = `Regal ${index} (${
      index % 2 === 0 ? "Rechts" : "Links"
    })`;
